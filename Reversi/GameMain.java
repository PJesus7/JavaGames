   ...
   /** Update the currentState after the player with "theSeed" has moved */
	...
	class DrawCanvas extends JPanel {
		...
		public void paintComponent(Graphics g) {  // invoke via repaint()
			...
			//construction of the red square
			g.setColor(Color.RED);
			g.fillRoundRect(Constants.CELL_SIZE * (Constants.ROWS/2-1), Constants.CELL_SIZE * (Constants.COLS/2+1)  - Constants.GRID_WIDHT_HALF,
			Constants.CELL_SIZE * 2, Constants.GRID_WIDTH, Constants.GRID_WIDTH, Constants.GRID_WIDTH);
			g.fillRoundRect(Constants.CELL_SIZE * (Constants.ROWS/2-1), Constants.CELL_SIZE * (Constants.COLS/2-1)  - Constants.GRID_WIDHT_HALF,
			Constants.CELL_SIZE * 2, Constants.GRID_WIDTH, Constants.GRID_WIDTH, Constants.GRID_WIDTH);

			g.fillRoundRect(Constants.CELL_SIZE * (Constants.ROWS/2-1) - Constants.GRID_WIDHT_HALF, Constants.CELL_SIZE * (Constants.COLS/2-1),
			Constants.GRID_WIDTH, Constants.CELL_SIZE * 2, Constants.GRID_WIDTH, Constants.GRID_WIDTH);
			g.fillRoundRect(Constants.CELL_SIZE * (Constants.ROWS/2+1) - Constants.GRID_WIDHT_HALF, Constants.CELL_SIZE * (Constants.COLS/2-1),
			Constants.GRID_WIDTH, Constants.CELL_SIZE * 2, Constants.GRID_WIDTH, Constants.GRID_WIDTH);
			} else {
				if(numberOfPieces) board.paintNumbers(g,currentPlayer); //if we are not on the first turns then we can have the number of pieces gained
			}
